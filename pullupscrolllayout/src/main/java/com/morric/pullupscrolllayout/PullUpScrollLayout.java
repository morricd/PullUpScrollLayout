...
package com.morric.pullupscrolllayout;
...
import com.morric.pullupscrolllayout.content.ContentScrollView;
 ...
 */
public class PullUpScrollLayout extends FrameLayout {
    ...
    public PullUpScrollLayout(Context context) {
        super(context);
    }

    public PullUpScrollLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        initFromAttributes(context, attrs);
    }

    public PullUpScrollLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initFromAttributes(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.PullUpScrollLayout);
        if (a.hasValue(R.styleable.PullUpScrollLayout_maxOffset_height)) {
            int maxset = a.getDimensionPixelOffset(R.styleable.PullUpScrollLayout_maxOffset_height, maxOffset);
            if (maxset != getScreenHeight()) {
                maxOffset = getScreenHeight() - maxset;
            }
        }
        if (a.hasValue(R.styleable.PullUpScrollLayout_minOffset_height))
            minOffset = a.getDimensionPixelOffset(R.styleable.PullUpScrollLayout_minOffset_height, minOffset);
        if (a.hasValue(R.styleable.PullUpScrollLayout_exitOffset_height)) {
            int exitset = a.getDimensionPixelOffset(R.styleable.PullUpScrollLayout_exitOffset_height, getScreenHeight());
            if (exitset != getScreenHeight()) {
                exitOffset = getScreenHeight() - exitset;
            }
        }
        if (a.hasValue(R.styleable.PullUpScrollLayout_allowHorizontalScroll))
            isAllowHorizontalScroll = a.getBoolean(R.styleable.PullUpScrollLayout_allowHorizontalScroll, true);
        if (a.hasValue(R.styleable.PullUpScrollLayout_isSupportExit))
            isSupportExit = a.getBoolean(R.styleable.PullUpScrollLayout_isSupportExit, true);
        if (a.hasValue(R.styleable.PullUpScrollLayout_mode)) {
            int mode = a.getInteger(R.styleable.PullUpScrollLayout_mode, 0);
            switch (mode) {
                case 0x0:
                    setToOpen();
                    break;
